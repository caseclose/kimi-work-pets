...
用法:
    python3 inspect_spritesheet.py <spritesheet.webp> [--cols 8] [--rows 9|11] [--out out_dir]
...
def main() -> int:
    ...
    parser.add_argument("--rows", type=int, default=None, help="默认按高度自动识别 9/11 行")
    ...
    rows = args.rows
    if rows is None:
        rows = h // 208 if h in (9 * 208, 11 * 208) else 9
    fw, fh = w // args.cols, h // rows
    print(f"size={w}x{h} frame={fw}x{fh} grid={args.cols}x{rows}")
    if fw * args.cols != w or fh * rows != h:
    ...
    sheet = Image.new("RGBA", (max(fw // 2, 1), max(fh // 2, 1) * rows), (255, 255, 255, 255))
